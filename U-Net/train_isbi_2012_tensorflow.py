...
from model_tf import UNET_TF_ISBI_2012
from loss_tf import binary_loss_object 

# seed 
tf.random.set_seed(1000)

flags.DEFINE_string('checkpoint_path', default='saved_model_isbi_2012/unet_model.ckpt', help='path to a directory to save model checkpoints during training')
...
class CustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(f'\nepoch after predict sampel : {epoch+1}\n')
...
def main(_):
    ...
    unet_model = UNET_TF_ISBI_2012(FLAGS.num_classes)
    ...
    # set callback
    model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(FLAGS.checkpoint_path, monitor='loss', verbose=1, save_best_only=True, save_weights_only=True)
...
if __name__ == '__main__':
    ...
    if gpus:
        try:
            # 필요할 때 메모리 증가를 허용하도록 설정
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
        except RuntimeError as e:
            # 프로그램 시작시에만 메모리 증가가 설정될 수 있음
            print(e)
    app.run(main)